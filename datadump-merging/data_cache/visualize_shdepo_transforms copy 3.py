...
def get_chain_to_root(node_id, parent_map):
    """Get all nodes from current node to root."""
    chain = [node_id]
    current = node_id
    while current in parent_map:
        current = parent_map[current]
...
def visualize_transforms(transforms, edges):
    ...
    def on_pick(event):
        ...
        # Get chain to root
        chain = get_chain_to_root(clicked_node, parent_map)
